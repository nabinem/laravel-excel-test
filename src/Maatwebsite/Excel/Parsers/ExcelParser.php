...
class ExcelParser {
    ...
    protected $orgIndices;
    
    protected $indicesMap;
    ...
    protected function getIndices()
    {
        ...
        foreach ($this->row->getCellIterator() as $this->cell) {
            $orgIndex = $this->getIndex($this->cell, true);
            $this->orgIndices[] = $orgIndex;
            $index = $this->getIndex($this->cell);
            $this->indices[] = $index;
            $this->indicesMap[$index] = $orgIndex;
        }
        // Return the labels
        return $this->indices;
    }
     ...
     * @return string
     */
    protected function getIndex($cell, $original = false)
    {
        // Get heading type
    ...
    protected function parseRows()
    {
        ...
        $parsedRows->setOriginalHeading($this->orgIndices);
        //set headings map
        $parsedRows->setHeadingsMap($this->indicesMap);
